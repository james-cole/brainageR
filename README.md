...
I am probably going to host this package on [Zenodo](https://zenodo.org/) or similar so that it has it's own DOI. I'll update this in due course, but I'd be grateful if you could cite this specific package that way. Watch this space.
...
```
## Installation
Currently this Github repo is missing a crucial file, the kernlab model file that actual contains the pre-trained GPR model. That because the model file, trained on N=2001 individuals is 5198MB in size, thus way over the limit for even Github LFS. You can either get this file directly from me (james.cole@kcl.ac.uk) or I'm working out a good place to host it, probably Zenodo.
